...
function _applyLeaveTypes(rows){
  if(!rows)return;
  // R26: leave types can now be deleted — honour the tombstone list so a row this device removed is
  // never re-created by a boot merge before the server DELETE lands (same rule as R7's other feeds).
  const _dead=new Set(DB.leaveTypes_deleted||[]);
  const mapped=_mLT(rows).filter(t=>!_dead.has(t.id));
  if(mapped.length||((DB.leaveTypes||[]).length===0))DB.leaveTypes=mapped;
}
...
/* R26 — attendance on demand for an arbitrary range. Boot loads 7 days and the Attendance tab
   cold-loads 90 (the R7 egress budget); the new From/To filter can reach further back, which would
   render an empty log that looks broken. Fetch only what is actually older than what we already
   hold, remember the earliest date fetched, and merge (never replace) so today's rows survive. */
let _attFromLoaded=null;
function _lazyAttFrom(fromISO){
  if(!S.uid||!fromISO)return;
  const cold=new Date(Date.now()-90*_DAY_MS).toISOString().slice(0,10);
  const floor=_attFromLoaded&&_attFromLoaded<cold?_attFromLoaded:cold;
  if(fromISO>=floor){_lazyCold('att90');return;}            // inside what we load anyway
  if(_tabLoading['attfrom'])return;
  _tabLoading['attfrom']=true;_syncBar(true);
  let q=sb.from('attendance').select('*').gte('date',fromISO);
  if(!(can('attendance','edit')||scopeOf('attendance')!=='self'))q=q.eq('user_id',S.uid);
  q.order('date',{ascending:false}).then(({data,error})=>{
    if(!error){_attFromLoaded=fromISO;_applyAttendance(data);saveDB();rr();}
    else console.warn('[attfrom]',error.message);
  }).catch(e=>console.warn('[attfrom]',e&&e.message))
   .then(()=>{_tabLoading['attfrom']=false;_syncBar(_anyLoading());});
}

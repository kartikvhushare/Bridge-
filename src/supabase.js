...
function _mDisc(r){return(r||[]).map(d=>({id:d.id,userId:d.user_id,level:d.level||'Verbal',reason:_unesc(d.reason)||'',note:_unesc(d.note)||'',issuedBy:d.issued_by||null,createdAt:d.created_at,expiresAt:d.expires_at||null}));}
function _discRow(d){return{id:d.id,user_id:d.userId,level:d.level,reason:d.reason||'',note:d.note||'',issued_by:d.issuedBy||null,created_at:d.createdAt||new Date().toISOString(),expires_at:d.expiresAt||null};}
function _mOT(r){return(r||[]).map(o=>({id:o.id,userId:o.user_id,date:o.date,hours:Number(o.hours||0),reason:_unesc(o.reason)||'',status:o.status||'Pending',comp:o.comp||null,decidedBy:o.decided_by||null,decidedAt:o.decided_at||null,decisionNote:_unesc(o.decision_note)||'',createdAt:o.created_at}));}
function _otRow(o){return{id:o.id,user_id:o.userId,date:o.date,hours:o.hours||0,reason:o.reason||'',status:o.status||'Pending',comp:o.comp||null,decided_by:o.decidedBy||null,decided_at:o.decidedAt||null,decision_note:o.decisionNote||'',created_at:o.createdAt||new Date().toISOString()};}
...
function _mSv(r){return(r||[]).map(s=>({id:s.id,kind:s.kind||'company',title:_unesc(s.title)||'',questions:Array.isArray(s.questions)?s.questions:[],runDate:s.run_date||null,status:s.status||'Active',createdBy:s.created_by||null,createdAt:s.created_at}));}
function _svRow(s){return{id:s.id,kind:s.kind,title:s.title||'',questions:s.questions||[],run_date:s.runDate||null,status:s.status||'Active',created_by:s.createdBy||null,created_at:s.createdAt||new Date().toISOString()};}

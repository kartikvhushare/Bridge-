...
function _aChartTheme(){
  // WEB-LOOK: same visual language as the marketing site — soft grid, brand greens, quiet neutrals.
  if(typeof Chart!=='undefined'&&!Chart.__bridged){Chart.__bridged=1;
    try{Chart.defaults.font.family="'Hanken Grotesk',system-ui,sans-serif";Chart.defaults.font.weight='600';Chart.defaults.color='#787D89';}catch(e){}}
  return {tick:'#8A93A3',grid:'rgba(138,147,163,0.14)',brand:'#0E9F6E',mint:'#34D399',soft:'#C9F3E3',neutral:'#E4E7EC',amber:'#F5A623',rose:'#F0716F',blue:'#38A8E8'};
}
...
function _drawHrmCharts(){
  ...
  mk('hrmChartWorked',noRows,{type:'bar',data:{labels:D.labels,datasets:[{label:'Worked hrs',data:D.worked,backgroundColor:'#0E9F6E',borderRadius:8,borderSkipped:false}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},onHover:_aHover,onClick:clk,scales:{x:{beginAtZero:true,ticks:{color:T.tick,font:{size:10}},grid:{color:T.grid}},y:{ticks:{color:T.tick,font:{size:11}},grid:{display:false}}}}});
  mk('hrmChartLate',noRows,{type:'bar',data:{labels:D.labels,datasets:[{label:'Lates',data:D.lates,backgroundColor:'#F5A623',borderRadius:8,borderSkipped:false},{label:'Absences',data:D.absences,backgroundColor:'#F0716F',borderRadius:8,borderSkipped:false}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:legB,onHover:_aHover,onClick:clk,scales:{x:{beginAtZero:true,ticks:{color:T.tick,font:{size:10},precision:0},grid:{color:T.grid}},y:{ticks:{color:T.tick,font:{size:11}},grid:{display:false}}}}});
  mk('hrmChartLeave',noRows,{type:'bar',data:{labels:D.labels,datasets:[{label:'Taken',data:D.taken,backgroundColor:'#0EA5E9',borderRadius:8,borderSkipped:false},{label:'Remaining',data:D.remaining,backgroundColor:'#E4E7EC',borderRadius:8,borderSkipped:false}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:legB,onHover:_aHover,onClick:clk,scales:{x:{stacked:true,beginAtZero:true,ticks:{color:T.tick,font:{size:10}},grid:{color:T.grid}},y:{stacked:true,ticks:{color:T.tick,font:{size:11}},grid:{display:false}}}}});
}
...
function _drawAnalyticsCharts(){
  ...
  const mk=(id,empty,cfg)=>{const cv=document.getElementById(id);if(!cv)return;if(empty){_emptyChart(id);return;}try{_aCharts.push(new Chart(cv.getContext('2d'),cfg));}catch(e){}};
  const legB={legend:{position:'bottom',labels:{color:T.tick,font:{size:11},boxWidth:12,padding:10}}};
  mk('aChartStatus',_allZero(A.status.data),{type:'doughnut',plugins:[_ctp],data:{labels:A.status.labels,datasets:[{data:A.status.data,backgroundColor:A.status.colors,borderWidth:0,hoverOffset:7,borderRadius:5,spacing:2}]},options:{responsive:true,maintainAspectRatio:false,cutout:'72%',_center:(()=>{const tot=A.status.data.reduce((a,b)=>a+b,0);const on=A.status.data[Math.max(0,A.status.labels.indexOf('On Time'))]||0;return{v:(tot?Math.round(on/tot*100):0)+'%',l:'on time'};})(),plugins:legB,onHover:_aHover,onClick:_aClick(i=>App._chartDrill('status',A.status.labels[i]))}});
  mk('aChartTrend',_allZero(A.trend.sub),{type:'line',data:{labels:A.trend.labels,datasets:[{label:'Submitted',data:A.trend.sub,borderColor:'#0EA5E9',backgroundColor:'rgba(14,165,233,0.12)',fill:true,tension:0.42,pointRadius:0,pointHoverRadius:4,borderWidth:2.5},{label:'On time',data:A.trend.ontime,borderColor:'#0E9F6E',fill:false,tension:0.42,pointRadius:0,pointHoverRadius:4,borderWidth:2.5},{label:'Late',data:A.trend.late,borderColor:'#EF4444',fill:false,tension:0.42,pointRadius:0,pointHoverRadius:4,borderWidth:2.5}]},options:{responsive:true,maintainAspectRatio:false,plugins:legB,onHover:_aHover,onClick:_aClick(i=>App._chartDrill('date',A.trend.labels[i])),scales:{x:{ticks:{color:T.tick,font:{size:10},maxRotation:0,autoSkip:true,maxTicksLimit:8},grid:{display:false}},y:{beginAtZero:true,ticks:{color:T.tick,font:{size:10},precision:0},grid:{color:T.grid}}}}});
  mk('aChartDept',!A.dept.labels.length,{type:'bar',data:{labels:A.dept.labels,datasets:[{label:'On time',data:A.dept.onTime,backgroundColor:'#0E9F6E',borderRadius:8,borderSkipped:false},{label:'Total',data:A.dept.total,backgroundColor:'#E4E7EC',borderRadius:8,borderSkipped:false}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:legB,onHover:_aHover,onClick:_aClick(i=>App._chartDrill('dept',A.dept.labels[i])),scales:{x:{beginAtZero:true,ticks:{color:T.tick,font:{size:10},precision:0},grid:{color:T.grid}},y:{ticks:{color:T.tick,font:{size:11}},grid:{display:false}}}}});
  mk('aChartTickets',_allZero(A.tickets.data),{type:'doughnut',plugins:[_ctp],data:{labels:A.tickets.labels,datasets:[{data:A.tickets.data,backgroundColor:A.tickets.colors,borderWidth:0,hoverOffset:7,borderRadius:5,spacing:2}]},options:{responsive:true,maintainAspectRatio:false,cutout:'72%',_center:{v:String(A.tickets.data.reduce((a,b)=>a+b,0)),l:'tickets'},plugins:legB,onHover:_aHover,onClick:_aClick(i=>App._chartDrill('ticket',A.tickets.labels[i]))}});
  mk('aChartEmp',!A.emp.labels.length,{type:'bar',data:{labels:A.emp.labels,datasets:[{label:'On time',data:A.emp.onTime,backgroundColor:'#0E9F6E',borderRadius:6,borderSkipped:false},{label:'Late',data:A.emp.late,backgroundColor:'#F0716F',borderRadius:6,borderSkipped:false},{label:'Pending',data:A.emp.pend,backgroundColor:'#F5A623',borderRadius:6,borderSkipped:false}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:legB,onHover:_aHover,onClick:_aClick(i=>{const id=A.emp.ids[i];if(id)App._userDrill(id);}),scales:{x:{stacked:true,beginAtZero:true,ticks:{color:T.tick,font:{size:10},precision:0},grid:{color:T.grid}},y:{stacked:true,ticks:{color:T.tick,font:{size:11}},grid:{display:false}}}}});
  mk('aChartCompliance',_allZero(A.compliance.data),{type:'doughnut',plugins:[_ctp],data:{labels:A.compliance.labels,datasets:[{data:A.compliance.data,backgroundColor:A.compliance.colors,borderWidth:0,hoverOffset:7,borderRadius:5,spacing:2}]},options:{responsive:true,maintainAspectRatio:false,cutout:'74%',_center:(()=>{const tot=A.compliance.data.reduce((a,b)=>a+b,0);return{v:(tot?Math.round((A.compliance.data[0]||0)/tot*100):0)+'%',l:'compliant'};})(),plugins:legB,onHover:_aHover,onClick:_aClick(i=>App._chartDrill('compliance',A.compliance.labels[i]))}});
}
function _drawHomeCharts(){
  ...
  const mk=(id,empty,cfg)=>{const cv=document.getElementById(id);if(!cv)return;if(empty){_emptyChart(id);return;}try{_aCharts.push(new Chart(cv.getContext('2d'),cfg));}catch(e){}};
  mk('hChartOnTime',_allZero(_HData.donut),{type:'doughnut',plugins:[_ctp],data:{labels:['On time','Late','Pending'],datasets:[{data:_HData.donut,backgroundColor:['#0E9F6E','#EF4444','#F59E0B'],borderWidth:0,hoverOffset:7,borderRadius:5,spacing:2}]},options:{responsive:true,maintainAspectRatio:false,cutout:'72%',_center:(()=>{const tot=_HData.donut.reduce((a,b)=>a+b,0);return{v:(tot?Math.round((_HData.donut[0]||0)/tot*100):0)+'%',l:'on time'};})(),plugins:{legend:{position:'bottom',labels:{color:T.tick,font:{size:11},boxWidth:12,padding:10}}}}});
  mk('hChartTrend',_allZero(_HData.done),{type:'line',data:{labels:_HData.labels,datasets:[{label:'Completed',data:_HData.done,borderColor:'#0E9F6E',backgroundColor:'rgba(14,159,110,0.12)',fill:true,tension:0.42,pointRadius:0,pointHoverRadius:4,borderWidth:2.5}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:T.tick,font:{size:10},maxRotation:0,autoSkip:true,maxTicksLimit:7},grid:{display:false}},y:{beginAtZero:true,ticks:{color:T.tick,font:{size:10},precision:0},grid:{color:T.grid}}}}});
}

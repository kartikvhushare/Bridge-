...
const HOW={
  reviews:{t:'Appraisal cycles: rate yourself, your manager rates you — both appear side by side when the cycle closes.',d:['Ratings run 1–10; comment rows are optional context.','Managers get one form per direct report; everyone gets a self review (when the cycle asks for it).','HR/Admin open & close cycles here and export results as CSV.','Probation confirmations (UAE Art 9) live here: confirm, extend (≤6 months from joining) or record a non-confirmation before probation ends.','Ratings are personal data — a notice shows on every form and data is kept only for the HR-Config retention window (PDPL).'],l:[['surveys','Surveys'],['hrmanalytics','HRM Analytics'],['users','Users']]},
  ...
  overtime:{t:'You log extra hours (auto-typed: normal, night 10pm–4am, or rest-day) → your manager approves. “Pay” lands in this month\'s payroll at the LEGAL rate; “Time-in-lieu” becomes comp-off leave instead.',d:['UAE Art 19 rates are floors: normal ×1.25, night & rest-day ×1.5 — HR Config can pay more, never less; the rate is frozen when approved.','Daily cap (2h, Art 19) and a weekly cap block over-logging.','Pending items also appear in the Approvals inbox.'],l:[['payroll','Payroll'],['leave','Leave'],['approvals','Approvals']]},
  ...
  discipline:{t:'Due process (UAE Art 39): open a written CHARGE → the colleague submits their STATEMENT → you decide a penalty from the statutory ladder → it stays on file 12 months, then auto-expires.',d:['Charge within 30 days of discovery; decide within 60 days of the statement — both windows are flagged.','Penalties: notice · warning · fine (≤5 days\' wage/mo, deducts in payroll) · suspension (≤14 days) · withhold bonus/promotion · dismissal (end-of-service preserved).','No penalty without hearing the employee (Cabinet Res 1/2022).'],l:[['users','Users'],['payroll','Payroll'],['audit','Audit']]},
  payroll:{t:'A full UAE engine that reads the month for you: paid days from clock-ins, sick & maternity at their legal tiers, overtime at legal rates, GPSSA pension for nationals, fines and adjustments — you just verify → HoP approves → Finalize.',d:['Salary structure is Basic + Housing + Transport + Other; gratuity and statutory overtime run on BASIC (Art 51/19); daily rate = gross ÷ 30 (MOHRE convention, changeable in HR Config).','Sick pay follows Art 31 automatically (15 days full → 30 half → rest unpaid, per year); maternity Art 30 (45 full + 15 half). The tiers live on the leave types.','Use the ± button on a draft line for bonuses, commissions or advance recoveries (one-off or recurring); deductions share the Art 25 50% cap with fines.','GPSSA: toggle per person in Users — 5%/11% employee by joining date, 15% employer, AED 3k–70k band, all editable.','A WPS chip tracks the pay-by-1st deadline (Res 340/2026); the export is a SIF-format bank file (EDR + SCR rows, MOL IDs from profiles).'],l:[['attendance','Attendance'],['leave','Leave'],['overtime','Overtime'],['users','Users'],['hrmconfig','HR Config']]},
  ...
  surveys:{t:'HR creates a survey with a run date → on that day the right people are notified: everyone rates the company, each person rates their manager, and managers rate each team member. Scores aggregate per person in HR Config → Surveys.',d:['Tick “Confidential” to hide respondent identity in results & exports (UAE PDPL) — recommended for “rate your manager”.','Personal responses are kept for the retention window set in HR Config → Alerts.'],l:[['hrmconfig','HR Config']]},
  okr:{t:'Create an objective with a target and a check-in day → the owner gets it as a task on that day → their numbers roll up the tree (L2 → L1 → L0) and the graph shows planned pace vs reality.',d:['Who sees what is set in Access Control (OKRs → “sees”): only their own, their team\'s, their department\'s, or everyone\'s. Owners always see their own objectives, and anything below a visible objective is visible too.','Turn on “Annual objective” in the editor to split it into quarterly targets — the quarters\' combined progress drives the annual number automatically.','Use the ✥ Move action to re-parent any objective (sub-objectives move with it), and 🔒 Close to freeze one for the record with a reason.','The quarter filter has Annual / Quarterly views — Quarterly mirrors the annual hierarchy (L1 Q1 under L0 Q1).'],l:[['mychecklists','My Checklists'],['dashboard','Dashboard'],['accesscontrol','Access Control']]},
};

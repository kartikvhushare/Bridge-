...
function payrollPage(){
  ...
  const rows=items.map(i=>{
    ...
    return `<tr style="border-top:1px solid var(--c-border);${d2.hold?'opacity:.5':''}">
      ...
      <td style="padding:8px 6px;font-size:12px;text-align:center;color:${i.unpaidDays?'var(--c-danger-ink)':'var(--c-text-3)'}">${i.unpaidDays||0}${d2.halfDays?`<span title="${d2.halfDays} half-pay day(s) — sick/maternity tiers (Art 31/30)" style="font-size:9.5px;color:#B45309;cursor:help"> +${d2.halfDays}½</span>`:''}</td>
      ...
      <td style="padding:8px 6px;font-size:11.5px;text-align:right">${(d2.adjAdds||d2.adjDed)?`<span title="Adjustments this month${d2.adjCapped?' · deduction capped at 50% (Art 25)':''}">${d2.adjAdds?'<span style="color:#0B7A55">+'+money(d2.adjAdds)+'</span>':''}${d2.adjAdds&&d2.adjDed?'<br>':''}${d2.adjDed?'<span style="color:#BE123C">−'+money(d2.adjDed)+'</span>':''}</span>`:'—'}</td>
      <td style="padding:8px 6px;font-size:12.5px;font-weight:800;text-align:right;color:var(--c-text)">${money(i.net)}${d2.fineAmount?` <span title="Includes disciplinary fine −${money(d2.fineAmount)} (Art 39 · ≤5 days/mo)" style="cursor:help">⚖</span>`:''}${d2.pensionEmp?` <span title="GPSSA pension −${money(d2.pensionEmp)} (${d2.pensionRate}% employee · employer pays ${money(d2.pensionEr)})" style="cursor:help">🏛</span>`:''}</td>
      ...
      <td style="padding:8px 10px;text-align:right;white-space:nowrap">${run.status==='Draft'&&can('payroll','run')?`<button onclick="App._payAdjModal('${i.userId}')" title="Additions & deductions for this person" class="ui-btn ui-btn-ghost ui-btn-sm" style="min-height:24px;padding:2px 8px;font-size:11px">±</button> `:''}<button onclick="App._payslip('${i.id}')" class="ui-btn ui-btn-ghost ui-btn-sm" style="min-height:24px;padding:2px 10px;font-size:11px">Payslip</button></td>
    </tr>`;
  }).join('');
  ...
  if(!run&&can('payroll','run'))actions=btnP('Create draft run — '+month,'App._payRun()','plus');
    ...
    else if(run.status==='Draft')actions=(can('payroll','run')&&items.length?btn('Recompute','App._payRecompute()',{variant:'ghost',size:'sm'}):'')+(can('payroll','verify')&&items.length?btn('Verify all','App._payVerifyAll()',{variant:'ghost',size:'sm',icon:'check'}):'')+'<span style="font-size:12px;color:var(--c-text-3);align-self:center">Verify every line (✓) to continue'+(can('payroll','run')?' · attendance up to the '+cutoff+' cut-off':'')+'</span>';
  ...
  return `<div class="fade">${hdr('Payroll','UAE engine — 30-day basis, sick/maternity tiers, GPSSA, adjustments · verify → approve → finalize → payslips + WPS SIF','')}
    ${_howBar('payroll')}
    ...
    <div class="ui-card" style="padding:0;overflow-x:auto"><table style="width:100%;border-collapse:collapse;min-width:820px">
      <thead><tr style="text-align:left"><th style="padding:9px 12px;font-size:10px;font-weight:800;color:var(--c-text-3);text-transform:uppercase">Person</th><th style="padding:9px 6px;font-size:10px;font-weight:800;color:var(--c-text-3);text-transform:uppercase;text-align:right" title="Basic + housing + transport + other allowances">Gross</th><th style="padding:9px 6px;font-size:10px;font-weight:800;color:var(--c-text-3);text-transform:uppercase;text-align:center">Present</th><th style="padding:9px 6px;font-size:10px;font-weight:800;color:var(--c-text-3);text-transform:uppercase;text-align:center">Unpaid d.</th><th style="padding:9px 6px;font-size:10px;font-weight:800;color:var(--c-text-3);text-transform:uppercase;text-align:right">OT</th><th style="padding:9px 6px;font-size:10px;font-weight:800;color:var(--c-text-3);text-transform:uppercase;text-align:right" title="One-off / recurring additions & deductions">Adj</th><th style="padding:9px 6px;font-size:10px;font-weight:800;color:var(--c-text-3);text-transform:uppercase;text-align:right">Net</th><th style="padding:9px 6px;font-size:10px;font-weight:800;color:var(--c-text-3);text-transform:uppercase;text-align:right">vs ${prev}</th><th style="padding:9px 6px;font-size:10px;font-weight:800;color:var(--c-text-3);text-transform:uppercase;text-align:center">✓</th><th></th></tr></thead>
      <tbody>${rows}</tbody>
    </table></div>`
...
/* ── R23 — additions & deductions ledger (per person). Draft-only edits; the line recomputes live. ── */
App._payReline=(userId)=>{ // recompute one person's line in the current Draft run
  const month=S.filters.pyMonth;const run=(DB.payrollRuns||[]).find(r=>r.month===month&&r.status!=='RolledBack');
  if(!run||run.status!=='Draft')return;
  const i=(DB.payrollItems||[]).find(x=>x.runId===run.id&&x.userId===userId);if(!i)return;
  const u=uById(userId);if(!u)return;
  Object.assign(i,_payCompute(u,month),{verified:false,verifiedBy:null});
  run.totals={net:(DB.payrollItems||[]).filter(x=>x.runId===run.id).reduce((a,x)=>a+x.net,0),people:run.totals?.people};
  _pushRow('payroll_items',_pItemRow(i),'payroll');_pushRow('payroll_runs',_pRunRow(run),'payroll');
};
App._payAdjModal=(userId)=>{
  if(!can('payroll','run'))return toast('You need Payroll → Run','err');
  const u=uById(userId);if(!u)return;_ensureHrm(u);
  const month=S.filters.pyMonth||todayISO().slice(0,7);
  const cur2=u.hrm?.salary?.currency||'AED';
  const list=(u.hrm.payAdjust||[]).filter(e=>e&&(e.month===month||e.recurring===true));
  const rows=list.map(e=>`<div style="display:flex;align-items:center;gap:8px;padding:7px 10px;border:1px solid var(--c-border);border-radius:10px;margin-bottom:6px">
      <span style="font-size:12.5px;font-weight:700;color:${e.kind==='deduct'?'#BE123C':'#0B7A55'}">${e.kind==='deduct'?'−':'+'}${cur2} ${(Math.round(Number(e.amount)*100)/100).toLocaleString()}</span>
      <span style="flex:1;font-size:12px;color:var(--c-text-2)">${esc(e.label||'')}</span>
      <span style="font-size:10px;font-weight:800;color:var(--c-text-3)">${e.recurring?'EVERY MONTH':month}</span>
      <button onclick="App._payAdjDel('${userId}','${e.id}')" style="border:none;background:transparent;color:var(--c-text-3);cursor:pointer">${ic('trash','w-3.5 h-3.5')}</button>
    </div>`).join('');
  modalShell({title:'Adjustments — '+esc(fullName(u)),sub:'Bonus, commission, reimbursement, advance recovery… Deductions share the Art 25 cap (≤50% of wage with fines).',size:'max-w-md',
    body:`<div>
      ${rows||'<div style="font-size:12px;color:var(--c-text-3);margin-bottom:8px">Nothing yet for '+month+'.</div>'}
      <div style="border-top:1px dashed var(--c-border);margin:10px 0"></div>
      <div style="display:flex;gap:8px;margin-bottom:8px">
        <select id="pa-kind" class="ui-select rf" style="width:130px"><option value="earn">+ Addition</option><option value="deduct">− Deduction</option></select>
        <input id="pa-amount" type="number" min="0" step="0.01" placeholder="Amount" class="ui-input rf" style="width:120px"/>
        <input id="pa-label" placeholder="Label — e.g. Sales commission" class="ui-input rf" style="flex:1"/>
      </div>
      <label style="display:flex;align-items:center;gap:8px;font-size:12.5px;font-weight:600;cursor:pointer"><input id="pa-rec" type="checkbox"/> Repeats every month (until removed)</label>
    </div>`,
    footer:btnG('Close','App.closeModal()')+btnP('Add',`App._payAdjSave('${userId}')`)});
};
App._payAdjSave=(userId)=>{
  if(!can('payroll','run'))return;
  const u=uById(userId);if(!u)return;_ensureHrm(u);
  const kind=document.getElementById('pa-kind')?.value==='deduct'?'deduct':'earn';
  const amount=Math.round((parseFloat(document.getElementById('pa-amount')?.value)||0)*100)/100;
  const label=(document.getElementById('pa-label')?.value||'').trim();
  const rec=!!document.getElementById('pa-rec')?.checked;
  if(!(amount>0))return toast('Enter the amount','err');
  if(!label)return toast('Give it a label','err');
  const month=S.filters.pyMonth||todayISO().slice(0,7);
  u.hrm.payAdjust=u.hrm.payAdjust||[];
  u.hrm.payAdjust.push({id:uid('pa'),kind,label,amount,month:rec?null:month,recurring:rec,at:new Date().toISOString(),by:S.uid});
  _acPushHrm(u);App._payReline(userId);
  log(fullName(me()),'Pay adjustment added',(kind==='deduct'?'−':'+')+amount+' '+label+' · '+fullName(u));
  saveDB();App._payAdjModal(userId);toast('Added — line recomputed');rr();
};
App._payAdjDel=(userId,entryId)=>{
  if(!can('payroll','run'))return;
  const u=uById(userId);if(!u||!u.hrm)return;
  u.hrm.payAdjust=(u.hrm.payAdjust||[]).filter(e=>e.id!==entryId);
  _acPushHrm(u);App._payReline(userId);
  log(fullName(me()),'Pay adjustment removed',fullName(u));
  saveDB();App._payAdjModal(userId);rr();
};
App._payRecompute=()=>{
  if(!can('payroll','run'))return toast('You need Payroll → Run','err');
  const month=S.filters.pyMonth;const run=(DB.payrollRuns||[]).find(r=>r.month===month&&r.status!=='RolledBack');
  if(!run||run.status!=='Draft')return toast('Only a Draft run can be recomputed','warn');
  const items=(DB.payrollItems||[]).filter(i=>i.runId===run.id);
  items.forEach(i=>{const u=uById(i.userId);if(u)Object.assign(i,_payCompute(u,month),{verified:false,verifiedBy:null});});
  run.totals={net:items.reduce((a,i)=>a+i.net,0),people:items.length};
  _pushRows('payroll_items',items.map(_pItemRow),'payroll');_pushRow('payroll_runs',_pRunRow(run),'payroll');
  log(fullName(me()),'Payroll draft recomputed',month);
  saveDB();toast('Draft recomputed from the latest data — re-verify the lines');rr();
};
App._payslip=(itemId)=>{
  const i=(DB.payrollItems||[]).find(x=>x.id===itemId);if(!i)return;
  ...
  const sal2=d2.salary||{basic:i.basic,housing:0,transport:0,other:i.allowances,gross:i.basic+i.allowances};
  const V={name:fullName(u),position:u.position||'—',department:u.department||'—',month:run.month,period:(d2.period?fmtD(d2.period.start)+' → '+fmtD(d2.period.end):run.month),currency:cur2,
    basic:n2(sal2.basic),housing:n2(sal2.housing),transport:n2(sal2.transport),other_allow:n2(sal2.other),gross:n2(sal2.gross),
    allowances:n2(i.allowances),ot_hours:d2.otHours||0,ot_amount:n2(i.otAmount),additions:n2(d2.adjAdds||0),
    unpaid_days:i.unpaidDays||0,half_days:d2.halfDays||0,per_day:n2(d2.perDay||0),deductions:n2(d2.absenceDed!=null?d2.absenceDed:i.deductions),
    other_deductions:n2(d2.adjDed||0),pension_emp:n2(d2.pensionEmp||0),pension_er:n2(d2.pensionEr||0),net:n2(i.net),
  ...
  const _DEF_TPL='PAYSLIP — {month}\nSalary period: {period}\n\nEmployee: {name}\nPosition: {position} · {department}\nStatus: {status}\n\n— Earnings —\nBasic salary:            {currency} {basic}\nHousing allowance:       {currency} {housing}\nTransport allowance:     {currency} {transport}\nOther allowances:        {currency} {other_allow}\nOvertime ({ot_hours}h):        {currency} {ot_amount}\nAdditions:               {currency} {additions}\n\n— Deductions —\nUnpaid/half days ({unpaid_days} + {half_days}½ × {per_day}): {currency} {deductions}\nOther deductions:        {currency} {other_deductions}\nGPSSA pension (employee): {currency} {pension_emp}\n\nNET PAY: {currency} {net}\n\n— Month summary —\nWorking days: {working} · Present: {present} · WFH: {wfh} · On leave: {leave} · Absent: {absent}\nLeave balance remaining: {leave_balance} days\n\n{note}\nGenerated on {date}';
  let body=String(B.payslipTpl||_DEF_TPL);
  Object.keys(V).forEach(k=>{body=body.split('{'+k+'}').join(String(V[k]));});
  const _adjList=(u.hrm?.payAdjust||[]).filter(e=>e&&(e.month===run.month||e.recurring===true));
  if(_adjList.length)body+='\n\n— Adjustments detail —\n'+_adjList.map(e=>(e.kind==='deduct'?'− ':'+ ')+cur2+' '+n2(e.amount)+'  '+(e.label||'')+(e.recurring?' (recurring)':'')).join('\n');
  if(d2.pensionEmp)body+='\n\nGPSSA: employee '+d2.pensionRate+'% = '+cur2+' '+n2(d2.pensionEmp)+' · employer '+cur2+' '+n2(d2.pensionEr)+' (paid by the company, not deducted).';
  if(d2.halfDays)body+='\nHalf-pay days follow the statutory sick/maternity tiers (Art 31/30).';
...
App._payWPS=()=>{
  ...
  /* R23 — SIF layout (EDR detail records + SCR control record), the format UAE banks/exchange houses
     ingest for WPS. MOL person IDs & routing codes come from each profile; employer IDs from
     HR Config → Compliance. Missing values export blank so you can spot & fill them. */
  const rows=_sifRows(run,items);
  const missing=items.filter(i=>{const u=uById(i.userId);return u&&(!u.hrm?.molId||!u.hrm?.iban);}).length;
  if(missing)toast(missing+' record(s) missing MOL ID or IBAN — fill them in Users → HRM','warn');
  _csvDownload(rows,'WPS_SIF_'+month);
  log(fullName(me()),'WPS SIF file exported',month);
};

...
function homeDash(){
  ...
  return `<div class="fade">
    ...
    ${_clockWidget()}
    <div style="display:flex;justify-content:flex-end;gap:8px;margin:-6px 0 14px"><button onclick="App._howModal()" title="How this entire tab works" style="display:inline-flex;align-items:center;gap:6px;padding:5px 12px;border-radius:20px;border:1.5px solid var(--c-border);background:var(--c-surface);color:var(--c-text-2);font-size:12px;font-weight:700;cursor:pointer">? How this works</button>${(()=>{const rec=(DB.attendance||[]).find(a=>a.userId===S.uid&&a.date===todayISO());const on=!!(rec&&(rec.flags||[]).includes('WFH'));return `<button onclick="App._togWFH()" style="display:inline-flex;align-items:center;gap:6px;padding:5px 12px;border-radius:20px;border:1.5px solid ${on?'#0EA5E9':'var(--c-border)'};background:${on?'#EFF6FF':'var(--c-surface)'};color:${on?'#0369A1':'var(--c-text-2)'};font-size:12px;font-weight:700;cursor:pointer">🏠 ${on?'Working from home today':'Mark today as WFH'}</button>`;})()}</div>
